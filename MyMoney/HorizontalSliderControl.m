...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
	...
	float offset = (point.x / 50.0f);
	int selected = ((offset - (int)offset) > 0.5) ? ceil(offset): floor(offset);
	
	[scrollView scrollRectToVisible:CGRectMake(selected * 50.0f, 0.0f, 320.0f, 60.0f) animated:YES];
	if (value != (selected + 1)) {
		value = selected + 1;
...
- (void) _initViews
{
	...
	sView.delegate = self;
	UIImage *counterImage = [UIImage imageNamed:@"counter3.png"];
